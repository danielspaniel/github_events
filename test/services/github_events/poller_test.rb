...
class GithubEvents::PollerTest < ActiveSupport::TestCase
  ...
  test "200 upserts events with actor and repo enrichment" do
    events = [github_push_event, github_push_event]
    ...
    actor = Actor.first
    assert_equal 14291370, actor.github_id
    assert_equal "jmartasek", actor.login
    assert_equal "https://api.github.com/users/jmartasek", actor.url
    ...
    assert_equal 1152169359, repository.github_id
    assert_equal "jmartasek/grafana", repository.name
    assert_equal "https://api.github.com/repos/jmartasek/grafana", repository.url
    ...
    event = GithubEvent.first
    assert_equal events.first["id"], event.event_id
    ...
    assert_equal 1152169359, event.repository_identifier
